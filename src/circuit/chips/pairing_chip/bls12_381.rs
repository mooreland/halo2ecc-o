...
where
    C: CurveAffine<Base = Fq>,
{
    fn fq2_mul_by_nonresidue(
        &mut self,
        a: &AssignedFq2<Fq, N>,
    ) -> Result<AssignedFq2<Fq, N>, Error> {
        let c0 = self.integer_context.int_sub(&a.0, &a.1).unwrap();
        let c1 = self.integer_context.int_add(&a.0, &a.1).unwrap();
...
impl<'a, C: CurveAffine, N: FieldExt> Fq6BnSpecificOps<Fq, N> for GeneralScalarEccContext<'a, C, N>
where
    C: CurveAffine<Base = Fq>,
{
...
impl<'a, C: CurveAffine, N: FieldExt> Fq12BnSpecificOps<Fq, N> for GeneralScalarEccContext<'a, C, N>
where
    C: CurveAffine<Base = Fq>,
{
    fn fq12_frobenius_map(
        ...
        x: &AssignedFq12<Fq, N>,
        power: usize,
    ) -> Result<AssignedFq12<Fq, N>, Error> {
...
impl<'a, C: CurveAffine, N: FieldExt> Fq2ChipOps<'a, Fq, N> for GeneralScalarEccContext<'a, C, N>
where
    C: CurveAffine<Base = Fq>,
{
    fn get_integer_context(&mut self) -> &mut IntegerContext<'a, Fq, N> {
        &mut self.integer_context
    }
}

impl<'a, C: CurveAffine, N: FieldExt> Fq6ChipOps<'a, Fq, N> for GeneralScalarEccContext<'a, C, N> where
    C: CurveAffine<Base = Fq>
{
}

impl<'a, C: CurveAffine, N: FieldExt> Fq12ChipOps<'a, Fq, N> for GeneralScalarEccContext<'a, C, N> where
    C: CurveAffine<Base = Fq>
{
}

impl<'a> GeneralScalarEccContext<'a, G1Affine, Fr> {
    fn ell(
        &mut self,
    ...
    fn cycolotomic_exp(&mut self, f: &AssignedFq12<Fq, Fr>) -> Result<AssignedFq12<Fq, Fr>, Error> {
        let x = BLS_X;
...
impl<'a> PairingChipOps<'a, G1Affine, Fr> for GeneralScalarEccContext<'a, G1Affine, Fr> {
    fn prepare_g2(
        &mut self,
    ...
    ) -> Result<AssignedFq12<<G1Affine as CurveAffine>::Base, Fr>, Error> {
